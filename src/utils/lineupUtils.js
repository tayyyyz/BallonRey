...
// Construye la alineación titular por defecto: intenta encajar por posición exacta,
// si no hay, por grupo (DEF/MID/FWD), priorizando mejor rating.
export function buildDefaultLineup(team, formationId) {
  ...
  // Paso 1: coincidencia exacta de posición
  ...
  // Paso 2: coincidencia por grupo (DEF/MID/FWD/GK)
  formation.slots.forEach((slot) => {
  ...
  // Paso 3: cualquier jugador restante disponible
  formation.slots.forEach((slot) => {
    if (lineup[slot.id]) return;
